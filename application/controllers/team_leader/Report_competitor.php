...
class Report_competitor extends REST_Controller {
	...
	function index_get() {
		if($this->get('id_tl')!=NULL){
			$q = "SELECT a.id_report_competitor,b.nama,a.foto,a.deskripsi,a.waktu FROM tb_report_competitor a LEFT JOIN tb_user b ON a.id_user=b.id_user WHERE a.id_user='".$this->get('id_tl')."' ORDER BY a.waktu DESC";
			...
			$q = "SELECT a.id_report_competitor,b.nama,a.foto,a.deskripsi,a.waktu FROM tb_report_competitor a LEFT JOIN tb_user b ON a.id_user=b.id_user WHERE a.id_report_competitor='".$this->get('id_report_competitor')."' ORDER BY a.waktu DESC";
			$get_data = $this->db->query($q)->result();
			if($get_data==NULL){
				echo "no_data";
			}else{
	...
	function index_post() {
		...
		$actual_path = "/home/aplikasi/public_html/intercipta/assets/tl_report/".$file_name;
