...
class Report_aktivitas extends REST_Controller {
	...
	function index_get() {
		if($this->get('id_user')!=NULL){
			$q = "SELECT a.id_report_aktivitas_fl AS id_laporan,a.id_user,b.nama,a.foto_outlet,a.keterangan,a.tgl FROM tb_report_aktivitas_fl a LEFT JOIN tb_user b ON a.id_user=b.id_user WHERE a.id_user='".$this->get('id_user')."'";
			...
			$q = "SELECT a.id_report_aktivitas_fl AS id_laporan,b.nama,a.id_user,a.foto_outlet,a.keterangan,a.tgl FROM tb_report_aktivitas_fl a LEFT JOIN tb_user b ON a.id_user=b.id_user WHERE a.id_report_aktivitas_fl='".$this->get('id_laporan')."'";
			$get_data = $this->db->query($q)->result();
			if($get_data==NULL){
				echo "no_data";
			}else{
	...
	function index_post() {
		...
		if (file_put_contents($actual_path, base64_decode($image)) != null) {
			$q = "INSERT INTO tb_report_aktivitas_fl(id_user,foto_outlet,keterangan,tgl) VALUES('".$this->post('id_user')."','".$actual_path2."','".$this->post('keterangan')."','".$this->post('tgl')."')";
	...
	function index_put() {
		...
		if($this->put('foto')==NULL){
			...
			if (file_put_contents($actual_path, base64_decode($image)) != null) {
				$data = array(
					'id_user'       => $this->put('id_user'),
					'foto_outlet'			=> $actual_path,
					'keterangan'    => $this->put('deskripsi'),
					'tgl'       => $this->put('tgl')
				);
				$this->db->where('id_report_aktivitas_fl', $this->put('id_laporan'));
				$update = $this->db->update('tb_report_aktivitas_fl', $data);
